// CONTENDER CLUB | SUPABASE VERSION 2026.08.12.2171 | MULTIPLES HORARIOS DE CLASE + RESERVAS DE ESPACIOS + TRAINER AUTH + ABONO POR VISITAS
// ═══════════════════════════════════════════════════════════════
// VERSION DEL ARCHIVO: 2026.08.12.2171 | supabasedb_v2026.08.12.2171.js | Contender Club | TRAINER AUTH + ABONO POR VISITAS
//
...
// Cambia la extensión de este archivo a .js y conserva el nombre: supabasedb_v2026.08.12.2171.js.
// ═══════════════════════════════════════════════════════════════

var GymDB = (function () {
  'use strict';

  var CONTENDER_SUPABASE_VERSION = '2026.08.12.2171';
  ...
  var DB_CONFIG = {
    ...
    DURACION_CLASE_HORAS_DEFAULT: 1,
    SOCIO_AUTH_DOMAIN: 'contenderclub.com'
  };
  ...
  function emailSocio(telefono,codigo){telefono=String(telefono||'').replace(/\D/g,'');codigo=String(codigo||'').trim();return telefono&&codigo?telefono+'.'+codigo+'@'+DB_CONFIG.SOCIO_AUTH_DOMAIN:'';}
  function emailSocioLegacy(telefono){telefono=String(telefono||'').replace(/\D/g,'');return telefono?telefono+'@'+DB_CONFIG.SOCIO_AUTH_DOMAIN:'';}
  ...
  function getPaginado(t,q,progreso,etiqueta){
    var out=[],offset=0,step=1000,base=String(q||'');
    function siguiente(){var qq=base+(base?'&':'')+'limit='+step+'&offset='+offset;return get(t,qq).then(function(r){if(r&&r._error)throw new Error(r.userMessage||r.message||('No se pudo descargar '+t));var rows=Array.isArray(r)?r:[];out=out.concat(rows);if(typeof progreso==='function')progreso(etiqueta||t,out.length);if(rows.length===step){offset+=step;return siguiente();}return out;});}
    return siguiente();
  }
  ...
  function normalizarHorarioClase(h){h=h||{};var d=h.dias;if(typeof d==='string'){try{d=JSON.parse(d);}catch(e){d=d.replace(/[{}\[\]\"']/g,'').split(',').map(function(x){return x.trim();}).filter(Boolean);}}if(!Array.isArray(d))d=[];h.id=h.id==null?null:String(h.id);h.clase_id=String(h.clase_id||'');h.hora=horaHHMM(h.hora);h.dias=d;h.cupo=Math.max(1,Number(h.cupo)||1);h.duracion_horas=Math.max(1,Math.floor(Number(h.duracion_horas)||DB_CONFIG.DURACION_CLASE_HORAS_DEFAULT));h.coach=String(h.coach||'');h.activo=h.activo!==false;return h;}
  ...
  function membresiaVigenteLocal(sid,fecha){sid=String(sid);fecha=String(fecha||hoy()).slice(0,10);var i=findIdx(C.socios,sid),s=i>-1?C.socios[i]:null;if(!s||s.activo===false)return null;var grupos={},candidatos=[];(s.abonos||[]).forEach(function(a){var ini=String(a.membresia_inicio||'').slice(0,10),fin=String(a.membresia_vencimiento||'').slice(0,10),plan=String(a.plan||'');if(!ini||!fin||!plan||planEsVisitas(plan)||fecha<ini||fecha>fin)return;var k=plan+'|'+ini+'|'+fin;if(!grupos[k]){grupos[k]={plan:plan,inicio:ini,fin:fin,unidad:'tiempo',pagos:[],total_abonado:0,ultimo_id:0};candidatos.push(grupos[k]);}grupos[k].pagos.push(a);grupos[k].total_abonado+=Number(a.monto)||0;grupos[k].ultimo_id=Math.max(grupos[k].ultimo_id,Number(a.id)||0);});candidatos.sort(function(a,b){return String(b.inicio).localeCompare(String(a.inicio))||String(b.fin).localeCompare(String(a.fin))||b.ultimo_id-a.ultimo_id;});if(candidatos.length)return candidatos[0];var plan=String(s.plan||''),ini=String(s.fecha_inicio||'').slice(0,10),fin=String(s.fecha_vencimiento||'').slice(0,10);if(plan&&planEsVisitas(plan)){var av=C.abonosVisitas.filter(function(x){return String(x.socio_id)===sid&&x.activo&&String(x.plan||'')===plan;}).sort(function(a,b){return Number(b.id)-Number(a.id);})[0];if(av)return{plan:plan,inicio:String(av.fecha_inicio||ini||'').slice(0,10)||null,fin:null,unidad:'visitas',abono_visitas_id:String(av.id),visitas_asignadas:Number(av.visitas_asignadas)||0,visitas_consumidas:Number(av.visitas_consumidas)||0,visitas_disponibles:Math.max((Number(av.visitas_asignadas)||0)-(Number(av.visitas_consumidas)||0),0),total_abonado:Number(av.monto_acumulado)||0};}if(plan&&fin&&(!ini||fecha>=ini)&&fecha<=fin){var pagos=(s.abonos||[]).filter(function(a){return(!a.plan||String(a.plan)===plan)&&(!a.membresia_inicio||String(a.membresia_inicio).slice(0,10)===ini)&&(!a.membresia_vencimiento||String(a.membresia_vencimiento).slice(0,10)===fin);}),total=pagos.reduce(function(t,a){return t+(Number(a.monto)||0);},0);return{plan:plan,inicio:ini||null,fin:fin,unidad:'tiempo',pagos:pagos,total_abonado:total,legacy:true};}return null;}
  ...
  function loadSocioActual(){
    ...
    return Promise.all([
      ...
      C.socios=[socio];
      C.checkins={};res[2].forEach(function(c){c.socio_id=String(c.socio_id||sid);c.tipo_checkin=String(c.tipo_checkin||'manual').toLowerCase()==='qr'?'qr':'manual';var f=String(c.fecha||'');if(!C.checkins[f])C.checkins[f]=[];C.checkins[f].push(c);});
  ...
  return {
    ...
    CLUB_TIMEZONE:DB_CONFIG.CLUB_TIMEZONE,
    DURACION_CLASE_HORAS_DEFAULT:DB_CONFIG.DURACION_CLASE_HORAS_DEFAULT,
    hoyClub:function(){return hoy();},
    getClubNow:function(){return deepCopyObj(ahoraClub());},
    ...
    loginSocio:function(telefono,codigo){
      ...
      var email=emailSocio(telefono,codigo),legacy=emailSocioLegacy(telefono),password=passwordSocio(telefono,codigo);
      function entrar(correo){return authCall('/token?grant_type=password',{email:correo,password:password});}
      return entrar(email).catch(function(e){var msg=e&&e.message?String(e.message):String(e);if(/invalid login credentials/i.test(msg)&&legacy&&legacy!==email)return entrar(legacy);throw e;}).then(function(s){
    ...
    getRespaldoCompletoDatos:function(progreso){
      if(!AUTH_SESSION||!AUTH_SESSION.access_token)return Promise.resolve({ok:false,error:'Se requiere una sesión de administrador para generar el respaldo completo'});
      var tareas=[
        getPaginado('socios','select=*&order=id.asc',progreso,'Socios'),
        getPaginado('abonos','select=*&order=socio_id.asc,fecha_pago.asc,id.asc',progreso,'Pagos de membresía'),
        getPaginado('checkins','select=*&order=fecha.asc,hora.asc,id.asc',progreso,'Check-ins'),
        getPaginado('ventas','select=*&tipo=eq.prod&order=id.asc',progreso,'Ventas'),
        getPaginado('membresias','select=*&order=id.asc',progreso,'Membresías'),
        getPaginado('productos','select=*&order=id.asc',progreso,'Productos'),
        getPaginado('clases','select=*&order=id.asc',progreso,'Clases'),
        getPaginado('clase_horarios','select=*&order=clase_id.asc,hora.asc,id.asc',progreso,'Horarios'),
        getPaginado('seguimiento_socio','select=*&order=socio_id.asc,fecha.asc,id.asc',progreso,'Composición corporal'),
        getPaginado('socio_abonos_visitas','select=*&order=socio_id.asc,id.asc',progreso,'Abonos por visitas'),
        getPaginado('trainers','select=*&order=id.asc',progreso,'Entrenadores')
      ];
      return Promise.all(tareas).then(function(r){return{ok:true,data:{socios:r[0],abonos:r[1],checkins:r[2],ventas:r[3],membresias:r[4],productos:r[5],clases:r[6],clase_horarios:r[7],seguimiento:r[8],abonos_visitas:r[9],trainers:r[10]}};}).catch(function(e){return{ok:false,error:e&&e.message?e.message:String(e)};});
    },
    ...
    getMembresiaVigente:function(id,fecha){var x=membresiaVigenteLocal(id,fecha);return x?deepCopyObj(x):null;},
    ...
    setSocio:function(socio){
      ...
      if(idx>-1){
        return pat('socios','id=eq.'+encodeURIComponent(socio.id),data).then(function(r){
          ...
          C.socios[idx]=socioCache;bump();return true;
        });
      }
    ...
    actualizarMembresiaActual:function(datos){
      var sid=String(datos&&datos.socio_id||''),body={p_socio_id:sid,p_plan:datos.plan||null,p_inicio:datos.fecha_inicio||null,p_vencimiento:datos.fecha_vencimiento||null,p_pagos:Array.isArray(datos.pagos)?datos.pagos:[]};
      return post('rpc/contender_actualizar_membresia_actual',body).then(function(r){if(!r)return{ok:false,error:'No hubo respuesta de Supabase'};if(r._error)return{ok:false,error:r.userMessage||r.message||'No se pudo actualizar la membresía actual'};var x=Array.isArray(r)?(r[0]||null):r;if(!x||x.ok===false)return{ok:false,error:x&&x.error?x.error:'No se pudo actualizar la membresía actual'};var si=findIdx(C.socios,sid);if(si>-1){var old={plan:x.plan_anterior||null,inicio:x.inicio_anterior||null,fin:x.vencimiento_anterior||null};function eraActual(a){if(!a)return false;if(a.membresia_inicio||a.membresia_vencimiento){if(a.plan&&old.plan&&String(a.plan)!==String(old.plan))return false;if(old.inicio&&a.membresia_inicio&&String(a.membresia_inicio)!==String(old.inicio))return false;if(old.fin&&a.membresia_vencimiento&&String(a.membresia_vencimiento)!==String(old.fin))return false;return true}if(a.plan&&old.plan&&String(a.plan)!==String(old.plan))return false;if(old.inicio&&a.fecha_pago&&String(a.fecha_pago)<String(old.inicio))return false;if(old.fin&&a.fecha_pago&&String(a.fecha_pago)>String(old.fin))return false;return true}var hist=(C.socios[si].abonos||[]).filter(function(a){return !eraActual(a)}),pagos=Array.isArray(x.pagos)?x.pagos:[];pagos.forEach(function(a){a.id=String(a.id);a.socio_id=sid;a.monto=Number(a.monto)||0;hist.push(a)});hist.sort(function(a,b){return(Number(a.numero)||0)-(Number(b.numero)||0)});C.socios[si].abonos=hist;C.socios[si].plan=x.plan||datos.plan||null;C.socios[si].fecha_inicio=x.fecha_inicio||datos.fecha_inicio||null;C.socios[si].fecha_vencimiento=x.fecha_vencimiento||datos.fecha_vencimiento||null;}bump();return{ok:true,data:x};});
    },
    eliminarMembresiaActual:function(sid){sid=String(sid);return post('rpc/contender_eliminar_membresia_actual',{p_socio_id:sid}).then(function(r){if(!r)return{ok:false,error:'No hubo respuesta de Supabase'};if(r._error)return{ok:false,error:r.userMessage||r.message||'No se pudo eliminar la membresía actual'};var x=Array.isArray(r)?(r[0]||null):r;if(!x||x.ok===false)return{ok:false,error:x&&x.error?x.error:'No se pudo eliminar la membresía actual'};var si=findIdx(C.socios,sid);if(si>-1){var old={plan:x.plan_anterior||null,inicio:x.inicio_anterior||null,fin:x.vencimiento_anterior||null};function eraActual(a){if(!a)return false;if(a.membresia_inicio||a.membresia_vencimiento){if(a.plan&&old.plan&&String(a.plan)!==String(old.plan))return false;if(old.inicio&&a.membresia_inicio&&String(a.membresia_inicio)!==String(old.inicio))return false;if(old.fin&&a.membresia_vencimiento&&String(a.membresia_vencimiento)!==String(old.fin))return false;return true}if(a.plan&&old.plan&&String(a.plan)!==String(old.plan))return false;if(old.inicio&&a.fecha_pago&&String(a.fecha_pago)<String(old.inicio))return false;if(old.fin&&a.fecha_pago&&String(a.fecha_pago)>String(old.fin))return false;return true}C.socios[si].abonos=(C.socios[si].abonos||[]).filter(function(a){return !eraActual(a)});C.socios[si].plan=null;C.socios[si].fecha_inicio=null;C.socios[si].fecha_vencimiento=null;}if(x.es_visitas)C.abonosVisitas=C.abonosVisitas.filter(function(a){return !(String(a.socio_id)===sid&&String(a.plan||'')===String(x.plan_anterior||''))});bump();return{ok:true,data:x};});
    },
    eliminarAbono:function(id){return Promise.resolve({ok:false,error:'El historial de pagos está protegido. Solo la membresía actual puede administrarse desde su ficha.'});},
    ...
    saveSeguimiento:function(socioId,mes,peso,musculo,imc,grasa,fecha,callback){var fechaFinal=fecha||hoy(),data={socio_id:socioId,mes:mes,peso:peso||null,musculo:musculo||null,imc:imc||null,grasa:grasa||null,fecha:fechaFinal};return get('seguimiento_socio','select=id&socio_id=eq.'+encodeURIComponent(socioId)+'&mes=eq.'+encodeURIComponent(mes)).then(function(r){if(r&&r._error)return{ok:false,error:r.userMessage||r.message||'No se pudo consultar el seguimiento'};var req=Array.isArray(r)&&r.length?pat('seguimiento_socio','id=eq.'+r[0].id,data):post('seguimiento_socio',data);return req.then(function(w){if(w&&w._error)return{ok:false,error:w.userMessage||w.message||'No se pudo guardar el seguimiento'};if(callback)callback();return{ok:true};});}).catch(function(e){return{ok:false,error:e&&e.message?e.message:String(e)};});},
    removeSeguimiento:function(id,callback){return del('seguimiento_socio','id=eq.'+id).then(function(r){if(r&&r._error)return{ok:false,error:r.userMessage||r.message||'No se pudo eliminar el seguimiento'};if(callback)callback();return{ok:true};}).catch(function(e){return{ok:false,error:e&&e.message?e.message:String(e)};});},
    ...
    addCheckin:function(sid,tipo){
      sid=String(sid);tipo=String(tipo||'manual').toLowerCase()==='qr'?'qr':'manual';var fechaLocal=hoy();
      return post('rpc/contender_registrar_checkin',{p_socio_id:sid,p_tipo_checkin:tipo,p_timezone:DB_CONFIG.CLUB_TIMEZONE}).then(function(r){
        if(!r)return {ok:false,error:'No hubo respuesta de Supabase'};
        if(r._error)return {ok:false,error:r.userMessage||r.message||'No se pudo registrar el check-in'};
        var x=Array.isArray(r)?(r.length?r[0]:null):r;if(!x||typeof x!=='object')return {ok:false,error:'Supabase no devolvio el check-in guardado'};
        var ci=x.checkin&&typeof x.checkin==='object'?x.checkin:x;ci.socio_id=String(ci.socio_id||sid);ci.tipo_checkin=String(ci.tipo_checkin||tipo).toLowerCase()==='qr'?'qr':'manual';var fecha=String(ci.fecha||fechaLocal),hora=String(ci.hora||ahoraHora()),arr=C.checkins[fecha]||[];ci.fecha=fecha;ci.hora=hora;
        var idx=arr.findIndex(function(z){return ci.id!=null&&String(z.id||'')===String(ci.id);});if(idx>-1)arr[idx]=ci;else arr.push(ci);C.checkins[fecha]=arr;
        var si=findIdx(C.socios,sid),nuevaVisita=x.nueva_visita!==false;if(si>-1){if(Number.isFinite(Number(x.visitas)))C.socios[si].visitas=Number(x.visitas);else if(nuevaVisita)C.socios[si].visitas=(C.socios[si].visitas||0)+1;if(nuevaVisita&&planEsVisitas(C.socios[si].plan)){var ev=C.abonosVisitas.filter(function(z){return String(z.socio_id)===sid&&z.activo&&String(z.plan)===String(C.socios[si].plan);}).sort(function(a,b){return Number(b.id)-Number(a.id);})[0];if(ev)ev.visitas_consumidas=Math.min(ev.visitas_asignadas,ev.visitas_consumidas+1);}}
        return consultarSaldoCheckin(sid,fecha).then(function(sr){
          ...
          bump();return {ok:true,checkin:deepCopy([ci])[0],tipo_checkin:ci.tipo_checkin,nueva_visita:nuevaVisita,actualizado_checkin:x.actualizado_checkin===true,saldo_sumado:x.saldo_sumado===true,visitas:Number.isFinite(Number(x.visitas))?Number(x.visitas):null,warning:sr&&sr.ok?null:(sr&&sr.error?sr.error:'El check-in se guardo, pero no se pudo consultar el saldo')};
        });
      });
    },
    ...
    guardarHorariosClase:function(cid,horarios){
      cid=String(cid);horarios=Array.isArray(horarios)?horarios:[];var payload=horarios.map(function(hh){return{id:hh.id==null||hh.id===''?null:+hh.id,hora:horaHHMM(hh.hora),dias:Array.isArray(hh.dias)?hh.dias:[],cupo:Math.max(1,Number(hh.cupo)||1),duracion_horas:Math.max(1,Math.floor(Number(hh.duracion_horas)||DB_CONFIG.DURACION_CLASE_HORAS_DEFAULT)),coach:String(hh.coach||'').trim()};});
...
ETAPA 12 TRAINER AUTH · VERSION 2026.08.12.2171
