// ═══════════════════════════════════════════════════════════════
// VERSION DEL ARCHIVO: 2026.08.11.1530 | supabasedb_v2026.08.11.1530.js | Contender Club | actualización 2026-08-11
//
...
// Después cambia la extensión de este archivo a .js y conserva el nombre: supabasedb_v2026.08.11.1530.js.
// ═══════════════════════════════════════════════════════════════

var GymDB = (function () {
  'use strict';

  var CONTENDER_SUPABASE_VERSION = '2026.08.11.1530';
    ...
    return new Promise(function(resolve){
      ...
      fetch(url,opts).then(function(r){
        ...
        if(!r.ok){
          ...
          if(table==='checkins'&&(r.status===409||code==='23505'))mensajeUsuario='⚠ Este socio ya tiene check-in hoy';
  ...
  function ahoraHora(){var d=new Date();return String(d.getHours()).padStart(2,'0')+':'+String(d.getMinutes()).padStart(2,'0')+':'+String(d.getSeconds()).padStart(2,'0');}
  ...
  return {
    ...
    addCheckin:function(sid){
      sid=String(sid);var fecha=hoy(),arr=C.checkins[fecha]||[];
      for(var i=0;i<arr.length;i++)if(String(arr[i].socio_id)===sid)return Promise.resolve({ok:false,duplicate:true,error:'El socio ya tiene check-in hoy'});
      var hora=ahoraHora();
      return post('checkins',{socio_id:sid,hora:hora,fecha:fecha}).then(function(r){
        if(!r)return {ok:false,error:'No hubo respuesta de Supabase'};
        if(r._error){var dup=String(r.code||'')==='23505'||Number(r.status)===409;return {ok:false,duplicate:dup,error:dup?'El socio ya tiene check-in hoy':(r.userMessage||r.message||'No se pudo registrar el check-in')};}
        var ci=Array.isArray(r)&&r.length?r[0]:null;if(!ci)return {ok:false,error:'Supabase no devolvió el check-in guardado'};
        ci.socio_id=String(ci.socio_id||sid);ci.fecha=ci.fecha||fecha;ci.hora=ci.hora||hora;arr.push(ci);C.checkins[fecha]=arr;
    ...
    saveVentas:function(arr){arr=(arr||[]).filter(function(v){return v.tipo==='prod';});var prev=C.ventas;C.ventas=arr;arr.forEach(function(v){if(!prev.some(function(p){return String(p.id)===String(v.id);}))post('ventas',{socio_id:v.socio_id,socio_nombre:v.socio_nombre,producto:v.producto,cantidad:v.cantidad,total:v.total,fecha:v.fecha,status:v.status,tipo:'prod',trainer:v.trainer||null}).then(function(r){if(r&&r[0])v.id=r[0].id;});});bump();},
