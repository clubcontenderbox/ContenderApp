// CONTENDER CLUB | SUPABASE VERSION 2026.08.12.2146 | SEGURIDAD ETAPA 4 | CARGA MINIMA SOCIO + AUTH ADMIN/SOCIO
// ═══════════════════════════════════════════════════════════════
// VERSION DEL ARCHIVO: 2026.08.12.2146 | supabasedb_v2026.08.12.2146.js | Contender Club | SEGURIDAD ETAPA 4 + 8D
//
// ETAPA 4 NO REQUIERE SQL NUEVO. El bloque SQL histórico al final se conserva
// solo como referencia de instalación/migraciones anteriores.
// Cambia la extensión de este archivo a .js y conserva el nombre: supabasedb_v2026.08.12.2146.js.
// ═══════════════════════════════════════════════════════════════

var GymDB = (function () {
  'use strict';

  var CONTENDER_SUPABASE_VERSION = '2026.08.12.2146';
  ...
  // ── SEGURIDAD ETAPA 4: carga mínima del socio autenticado ──
  // Nunca descarga todos los socios, ventas, abonos, check-ins, saldo o cuentas.
  // Los catálogos de membresías/clases siguen siendo públicos de lectura por diseño.
  // Para cupos de clases solo baja clase_id + fecha; no baja IDs de otros socios.
  function loadSocioActual(){
    ...
    return Promise.all([
      ...
      get('inscritos','select=clase_id,fecha&fecha=gte.'+f0),
      ...
      C.cuposClases={};res[7].forEach(function(i){var f=i.fecha||f0,key=String(i.clase_id)+'|'+f;C.cuposClases[key]=(C.cuposClases[key]||0)+1;});
  ...
  return {
    ...
    inscribirSocio:function(cid,sid,fecha){
      ...
      return post('inscritos',{clase_id:+cid,socio_id:sids,fecha:f}).then(function(r){
        if(r&&r._error){var dup=String(r.code||'')==='23505'||Number(r.status)===409;return{ok:false,duplicate:dup,error:dup?'Ya estás inscrito en esta clase':(r.userMessage||r.message||'No se pudo guardar la inscripción')};}
        C.inscritos[key].push(sids);if(SOCIO_AUTH_SESSION)C.cuposClases[key]=(Number(C.cuposClases[key])||0)+1;bump();return{ok:true,data:Array.isArray(r)&&r.length?deepCopy(r)[0]:null};
      });
    },
    desinscribirSocio:function(cid,sid,fecha){
      ...
      return del('inscritos','clase_id=eq.'+encodeURIComponent(cid)+'&socio_id=eq.'+encodeURIComponent(sids)+'&fecha=eq.'+encodeURIComponent(f)).then(function(r){
        ...
        if(C.inscritos[key])C.inscritos[key]=C.inscritos[key].filter(function(x){return String(x)!==sids;});if(SOCIO_AUTH_SESSION)C.cuposClases[key]=Math.max(0,(Number(C.cuposClases[key])||0)-1);bump();return{ok:true};
      });
    },
...
-- 2.0.1) SEGURIDAD 2145: saldo_movimientos para sesiones Auth.
-- Desde que el admin usa Supabase Auth, las peticiones REST se ejecutan como
-- rol authenticated. Las politicas antiguas solo autorizaban anon, por lo que
-- el admin podia ver saldo 0 aunque los movimientos existieran en PostgreSQL.
-- Admin: lectura y escritura. Socio: solo lectura de sus propios movimientos.
grant select, insert, update, delete on table public.saldo_movimientos to authenticated;
grant usage, select on sequence public.saldo_movimientos_id_seq to authenticated;

-- Helper SECURITY DEFINER: permite resolver el socio del JWT sin abrir
-- public.socio_auth al cliente.
create or replace function public.contender_socio_actual_id()
returns text
language sql
stable
security definer
...
as $$
  select sa.socio_id
  from public.socio_auth sa
  where sa.auth_user_id = auth.uid()
    and sa.activo = true
  limit 1;
$$;
revoke all on function public.contender_socio_actual_id() from public;
revoke all on function public.contender_socio_actual_id() from anon;
grant execute on function public.contender_socio_actual_id() to authenticated;

drop policy if exists "saldo_movimientos_select_authenticated" on public.saldo_movimientos;
create policy "saldo_movimientos_select_authenticated" on public.saldo_movimientos
  for select to authenticated
  using (
    public.contender_es_admin()
    or saldo_movimientos.socio_id = public.contender_socio_actual_id()
  );

drop policy if exists "saldo_movimientos_insert_authenticated_admin" on public.saldo_movimientos;
create policy "saldo_movimientos_insert_authenticated_admin" on public.saldo_movimientos
  for insert to authenticated
  with check (public.contender_es_admin());

drop policy if exists "saldo_movimientos_update_authenticated_admin" on public.saldo_movimientos;
create policy "saldo_movimientos_update_authenticated_admin" on public.saldo_movimientos
  for update to authenticated
  using (public.contender_es_admin())
  with check (public.contender_es_admin());

drop policy if exists "saldo_movimientos_delete_authenticated_admin" on public.saldo_movimientos;
create policy "saldo_movimientos_delete_authenticated_admin" on public.saldo_movimientos
