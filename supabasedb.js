// ═══════════════════════════════════════════════════════════════
// supabasedb.js · Contender Club · actualización 2026-08-07
//
...
var GymDB = (function () {
  ...
  function sbFetch(method,table,query,body){
    ...
    return fetch(url,opts).then(function(r){
      ...
      return r.json().then(function(json){
        if(!r.ok){
          var msg=(json&&json.message)?json.message:JSON.stringify(json),code=json&&json.code?String(json.code):'',constraint=json&&json.details?String(json.details):'';
          var mensajeUsuario='⚠ DB '+r.status+': '+msg.slice(0,80);
          if(table==='membresias'&&(r.status===409||code==='23505')&&(msg.indexOf('membresias_nombre_key')>-1||constraint.indexOf('nombre')>-1))mensajeUsuario='⚠ Ya existe una membresía con ese nombre';
  ...
  return {
    ...
    saveMembresias:function(arr){_syncCat('membresias',C.membresias,arr,['nombre','unidad','cantidad','dias','precio','descripcion']);C.membresias=arr;bump();},
